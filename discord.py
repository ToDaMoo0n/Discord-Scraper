...
def startGuild(scraper, guild, channel, from_day=None):
    if from_day == None:
        raise "from_day is a required argument"

    # Get the snowflakes for the current day.
    snowflakes = DiscordScraper.getDayBounds(from_day.day, from_day.month, from_day.year)

    # Generate a valid URL to the undocumented API function for the search feature.
    messageUrl = 'https://discord.com/api/{0}/channels/{1}/messages?limit=100'.format(scraper.apiversion, channel)

    # Update the HTTP request headers to set the referer to the current guild channel URL.
    # scraper.headers.update({'Referer': 'https://discord.com/channels/{0}/{1}'.format(guild, channel)})

    try:
        response = DiscordScraper.requestData(messageUrl, scraper.headers)
        if response is None:
            print("no data found for days {}")
...
def start(scraper, guild, channel, from_date=None, to_date=None):
    if from_date == None:
        raise "from_date is a required argument"

    if to_date == None:
        to_date = datetime.today()
    
    # Determine if the year is no less than 2015 since any time before this point will be guaranteed invalid.
    if from_date.year <= 2014:
        exit(0)

    query_date = from_date

    while query_date.timestamp() <= to_date.timestamp():
        data = startGuild(scraper, guild, channel, from_date)
...
if __name__ == '__main__':
    ...
    for guild, channels in discordscraper.guilds.items():
        for channel in channels:
            print(f"♥️ CHANNEL {channel} {from_date}")

            # Start the scraper for the current channel.
            start(discordscraper, guild, channel, from_date)
